...
- 🔴🔴 **股价与推荐防错铁律（08-07 落地·根因=8/6早报选股价数量级错误）**：**报告/选股/持仓中所有股价与买区不允许出错**。①选股段价位**必须**由 `advisor_rules.py check-entry --code X` 脚本取价（gtimg实时+MA20+52周），**禁止 AI 手填未经校验数字**；②`scripts/price_sanity.py` 三闸门(G1实时偏差>30%/G2 52周区间/G3 MA20偏离>60%)任一失败→`SANITY_FAIL`+自动改用可信价；**支持美股**(`--market us` 走 Yahoo 实时+52周，G3跳过)；③`check_entry` 已改：外部价必经 sanity 校验，失败→`blocked=True`绝不输出离谱买区；④早报1782741941693+晚报1782741945710/1782817769722 prompt 已嵌价格防错铁律，`price_sanity.ok=false` 标的不得进建仓清单标「🚫价格校验失败，已拦截」。**⑤盘中监控全覆盖（08-07）**：`fetch_holdings_quotes.py` 已加 `_apply_sanity()`——实盘/模拟盘取到的每只 `current_price` 必经 sanity，失败标 `price_sanity_fail=true`+回填 `reliable_current_price`，顶层 `sanity_failed` 计数；6个盘中自动化(实盘1784039316540+投顾5策略1784506600526/634/523/665/706) prompt 加「现价防错铁律」。**⑥美股监控1780615006148 铁律**：AAPL/TSLA/NVDA 收盘价必须过 `price_sanity --market us`，FAIL→标注「🚫价格校验失败，已隔离」+重搜；禁手填美股价。**⑦跨盘监控 cross_portfolio_monitor.py 二次校验**：`_sanity_guard()` 对 portfolio.json 读的 current_price 做 sanity，失败→隔离错误价(不计入总市值)+`sanity_failed`计数。**⑧工程质量周报1782002834355 PHASE2.5 已加 `tests/test_price_sanity.py`(12用例)+跨盘测试 回归门禁**。**⑨sim_trade.py 交易执行校验（最高优先级）**：`.workbuddy/scripts/sim_trade.py` 新增 `_sanity_check_price()`，在 `cmd_update_price`/`cmd_update_all_prices`(错误价拒绝写入保留旧价) + `cmd_buy`/`cmd_sell`(错误成交价拒绝交易绝不污染avg_cost/已实现盈亏) 四个入口拦截。**⑩drill_assistant_monitor.py 消费 sanity**：PHASE3 分级前过滤 `price_sanity_fail=true` 标的→隔离告警+不参与止损盈亏判定+市值汇总排除。DO NOT REVERT：禁回退到"AI 直接写买区/现价"的旧逻辑。
...
- 🔴🔴 **创建/修改自动化 rrule 强制自检 gate（08-07 二次踩坑后固化）**：凡涉及 `automation_update` 创建/更新或直写 db automations 表且 rrule 含 `BYHOUR` 多值 → **必须先 load `automation-rrule-safety-check` skill 走 Gate1-4**（禁多BYHOUR拆多条 / 备份 / 创建后验证创建数+单BYHOUR+告知用户当天剩余时段是否排得上 / 回溯查昨天同坑一并修）。不可跳过、不可只验证"创建成功+平台可见"。这是信任红线——用户明确"信任你才让你操作"，复发即严重失职。
...
- 🔴自动化排程铁律(07-27)：**单RRULE禁多BYHOUR**（只触发首个匹配小时，余槽静默丢无日志）→多时段拆多条单BYHOUR。已拆：助理实盘1784039316540=9 + 1785123941471/596/709/786(10/11/13/14)；信号溯源1780964240621=5 + 1785284629106(15:00)；原隐患1783310235388已删
  - 🔴🔴 **二次踩坑(08-06→08-07)：连续两次犯同错**！08-06 合并的"助理实盘监控/投顾策略执行"两条合并版(rrule=`BYHOUR=9,10,11,13,14`单条)踩此坑→今天(08-07)只跑09:00/09:10、余4槽全丢，用户感知"盘中监控没发"才发现。08-07 10:54 已拆成10条单BYHOUR(1786071284155/157/159/161/163 + 1786071284165/167/169/171/173，宿主QTS)并软停2合并版。**血训：①建/改 rrule 前必须走 `automation-rrule-safety-check` skill 强制自检 ②"今天剩余时段能否排上"必须验证并告知用户，不能只验证"创建成功+平台可见" ③修昨天新建的同类自动化时要回溯查是否同坑、一并修**
