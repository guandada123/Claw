...
def validate_qts_signals(data: dict, max_age_hours: int = 8,
    ...
    ⚠️ DO NOT REVERT（2026-09-03 加）：必须**同时**校验两个时间维度，只查一个等于没查。
    - generated_at = 「我什么时候去拉的」→ 只能证明拉取动作新鲜
    - report_date  = 「拉到的数据本体是哪天的」→ 才是信号的真实时效

    历史实证：QTS backtest_reports 自 2026-07-16 起断更 47 天，而
    pull_qts_signals.py 取 `ORDER BY created_at DESC LIMIT 1`（永远有行、不报错），
    于是 09-01 15:05 拉到 07-16 的日报、generated_at 全新 → 本函数原版判「✅ 契约有效」
    放行。若那份陈旧日报含 WF 通过的个股，49 天前的选股会被当今日信号进入共识。
    根因是 report_date 此前只被读进返回值、从不参与判定（装饰性字段）。
    ...
    max_report_age_days 默认 10：日报仅交易日生成，需容忍春节/国庆长假造成的
    9~10 个日历日断档，避免长假复市首日误报。
...
def _load_qts_signals() -> dict:
    ...
    # ⚠️ DO NOT REVERT（2026-09-03 加）：report_stale 必须**拦截**而非仅告警。
    # 只置 note 等于只修一半 —— 陈旧选股照样进共识，note 无人消费。
    # 与 generated_at 的 stale 刻意不对称：stale 是「拉取时间偏旧」（05:05 拉、
    # 14:00 消费即超 8h，属常态，故沿用 note-only）；report_stale 是「上游断更、
    # 数据本体作废」，属结构性故障，必须像 quarantine 一样拒收。
    if v.get("report_stale"):
        return {"signals": [], "note": f"⚠️ QTS信号已拒收: {v['msg']}"}
    if not v["ok"]:
