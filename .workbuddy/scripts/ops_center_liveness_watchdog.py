...
设计意图：
  ...
  若中枢自身挂死/静默失败，该时间戳不再更新。本看门狗独立调度（每6h，
  2026-09-04 P0-C 运维层降频：2h→6h），读取 last_ok_ts 算间隔，
  超过阈值则飞书告警"中枢可能失联"。

  阈值不变式：threshold = 3 × 中枢调度周期。
  2026-09-04 P0-C 降频前中枢为 60min → 阈值 180min；
  降频后中枢为 180min（FREQ=HOURLY;INTERVAL=3）→ 阈值同步放大为 540min。
  中枢调度周期若再变，本常量必须同步，否则会系统性误报。
...
用法：
  ...
  python3 ops_center_liveness_watchdog.py --threshold-min 540
"""
...
DEFAULT_THRESHOLD_MIN = 540  # 3倍中枢调度周期(180min，2026-09-04 P0-C 降频后)
...
def main() -> int:
    ...
    if not last_ok:
        ...
        if args.dry_run:
            print("[liveness-watchdog] [dry-run] 跳过实际推送")
        else:
            ok = _push_card(
                "🛡️ 巡检中枢存活看门狗",
            ...
            print(
                f"[liveness-watchdog] 飞书推送结果: "
                f"{'送达' if ok else '❌ 未送达（需人工核验通道）'}"
            )
        return 0
