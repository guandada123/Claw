...
MODEL_PRICES = {
    ...
    # Hy4 preview（2026-08-28 上线）：WorkBuddy 侧限时免费至 2026-09-10 23:59，
    # 每日有额度上限，触顶后消耗积分/繁忙排队。免费期结束后（2026-09-11 起）按官方
    # API 定价 6元/百万输入、18元/百万输出（=0.06/万、0.18/万）计费。
    # [2026-09-11 更新] 免费期已结束，已把两值改为 {"input": 0.06, "output": 0.18}。
    "hy4-preview": {"input": 0.06, "output": 0.18},
...
# ============================================================
# Agent 模型（WorkBuddy 自动化会话模型）消耗回算
# ============================================================
# 背景：自动化的会话模型（hy3 / hy4-preview 等）不走 router.py，
# 其 token 消耗从未进入 ~/.ai_cost_log.jsonl，导致月预算闸存在盲区。
# 数据源：~/.workbuddy/workbuddy.db
#   - automation_runs.metadata_json.conversationId → session_id
#   - session_usage.used = 该会话最终上下文占用 token
# 估算口径（保守下界）：session_usage.used = 会话最终上下文占用（含系统提示词），
# 而 agent 多轮循环每轮都会重发上下文，实际计费输入 token 数倍于此，
# 故乘 AGENT_TOKEN_FACTOR；输出按输入的 10% 估。
# ⚠️ 仅作量级参考：平台若按积分/订阅计费，实际扣费以平台账单为准。
AGENT_TOKEN_FACTOR = 3.0
AGENT_OUTPUT_RATIO = 0.10

AUTOMATION_DB = Path.home() / ".workbuddy" / "workbuddy.db"


def collect_agent_usage(days: int = 1) -> dict:
    """
    从 workbuddy.db 汇总最近 N 天「自动化会话模型」的 token 消耗（只读）。

    口径：sessions.is_background_automation=1 的会话 × session_usage.used。
    （曾用 automation_runs.metadata_json.conversationId 关联，覆盖率仅 3%，已废弃。）
    """
    if not AUTOMATION_DB.exists():
        print(f"⚠️ 未找到自动化库: {AUTOMATION_DB}")
        return {}

    since_ms = int(time.time() * 1000) - days * 86_400_000
    con = sqlite3.connect(f"file:{AUTOMATION_DB}?mode=ro", uri=True)
    cur = con.cursor()
    cur.execute(
        """
        SELECT s.model                            AS model,
               COUNT(*)                           AS runs,
               SUM(u.used IS NOT NULL)            AS covered,
               SUM(CAST(u.used AS INTEGER))       AS tokens
        FROM sessions s
        LEFT JOIN session_usage u ON u.session_id = s.id
        WHERE s.is_background_automation = 1
          AND CAST(s.created_at AS INTEGER) > ?
        GROUP BY s.model
    ...
    out = {}
    for model, runs, covered, tokens in rows:
        if not model:
            continue
        out[model] = {
            "runs": runs,
            ...
            "est_input": int((tokens or 0) * AGENT_TOKEN_FACTOR),
            "est_output": int((tokens or 0) * AGENT_TOKEN_FACTOR * AGENT_OUTPUT_RATIO),
        }
    return out


def agent_rollup(days: int = 1) -> dict:
    """
    汇总自动化 agent 会话模型的 token 体量（只读，不写成本日志）。

    ⚠️ 不输出 ¥ 金额：WorkBuddy 平台对会话模型按积分/订阅计费，费率未知，
    按 MODEL_PRICES 的 API 官方价折算会高估 1~2 个数量级（实测：hy3 近7天
    4,958万上下文 token，按 ¥2/万 折算得 ¥3.8万/周，与实际 ¥400/月 支出差
    两个数量级）。故此处只报体量，费率确认后再接入预算闸。
    """
    usage = collect_agent_usage(days)

    print(f"\n🤖 自动化 Agent 会话模型 — token 体量（近 {days} 天）")
    print("─" * 72)
    if not usage:
        print("  无数据（该时间窗内没有自动化会话）")
        return {}

    total_ctx = 0
    total_runs = 0
    width = max([len(m) for m in usage] + [10])
    for model, u in usage.items():
        total_ctx += u["context_tokens"]
        total_runs += u["runs"]
        print(
            f"  {model:<{width}} 会话={u['covered']}/{u['runs']:<5} "
            f"上下文={u['context_tokens']:>12,} "
            f"折算输入≈{u['est_input']:>13,}"
        )
    print("─" * 72)
    print(f"  合计: {total_runs} 个会话 / 上下文 {total_ctx:,} token")
    if days:
        print(f"  日均: {total_runs / days:.0f} 个会话 / 上下文 {total_ctx / days:,.0f} token")
    print(
        f"  ⚠️ 口径: 上下文占用（含系统提示词）×{AGENT_TOKEN_FACTOR} 折算计费输入，属保守下界\n"
        "     ❗不输出金额：平台按积分/订阅计费，非 API 官方价，折算会高估1~2个数量级"
    )
    return usage
...
if __name__ == "__main__":
    ...
    elif cmd == "agent_rollup":
        # 汇总自动化 agent 会话模型 token 体量（只读，不写成本日志）
        # 用法: python3 cost_tracker.py agent_rollup [days]
        days = 1
