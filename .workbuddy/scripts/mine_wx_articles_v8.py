...
def main():
    ...
    print(
        f"[gate] 总文章 {len(files)} | 已处理 {len(processed)} | 本次处理 {len(new_files)}"
    ...
    print(
        f"[archive] 提取 {len(new_signals)} 条候选 | 去重后新增 {len(truly_new)} 条 | 累计 {len(all_signals)} 条"
    )
    ...
    if len(quality) >= 2:
        ...
        lines = [
            f"📚知识库挖掘 | 增量归档 {len(truly_new)} 条信号（优质 {len(quality)}）",
            "━━━━━━━━━━━━",
        ]
        for acc in sorted(q_by_acc, key=lambda a: -len(q_by_acc[a])):
            ...
            for s in q_by_acc[acc][:4]:
                arrow = (
                    "看多"
                    if s["signal"] == "bullish"
                    else ("看空" if s["signal"] == "bearish" else "中性")
                )
                lines.append(
                    f"  · {s['stock_name']}({s['stock_code']}) {arrow} — 《{s['title'][:18]}》"
                )
