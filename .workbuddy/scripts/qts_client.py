...
def _query(sql: str, params: tuple | None = None, max_rows: int | None = None):
    ...
    2026-09-04 加固: 异常时 rollback + close 当前连接再丢弃，避免 colima 端口
    转发偶发抖动后缓存连接卡在 aborted 事务态、导致后续重试在同一污染连接上
    全部失败（InFailedSqlTransaction）。每次重试强制新建连接。
    """
...
def get_daily_report() -> dict | None:
    ...
    2026-09-04 修复（数据新鲜度契约错位根因）:
    QTS report_scheduler 把 top_strategies 落进结构化列 `strategies_covered`(始终 JSON，
    注意列名是 strategies_covered 不是 covered；report_scheduler.py:545 把 :covered
    参数写进 strategies_covered 列)，detail_content 是 Markdown 正文
    （models.py:254 注释明写「报告正文 Markdown」）。
    原实现只取 detail_content 并 json.loads → Markdown 解析失败 → Claw 端
    pull_qts_signals 守卫生效、陈旧文件被永久保留（63h+ 陈旧告警）。
    现改取 `strategies_covered` 作为 top_strategies 主源；
    detail 仍解析以备极偶然的 JSON 回灌（含 wf_validated）时优先采用。
    """
    rows = _query(
        """
        SELECT report_type, report_date, detail_content, strategies_covered, summary
    ...
    if isinstance(detail, str):
        try:
            ...
            pass
    covered = r.get("strategies_covered")
    ...
    return {
        ...
        "detail": detail,
        "top_strategies": covered if isinstance(covered, list) else [],
        "summary": summary,
    }
