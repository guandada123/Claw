...
# ============================================================
try:
    from cost_tracker import _match_model, MODEL_PRICES, log_call as _log_call
except ImportError:
    from scripts.cost_tracker import _match_model, MODEL_PRICES, log_call as _log_call

try:
    from local_model import call as _local_call, is_available as _local_check
    _LOCAL_AVAILABLE = True
except ImportError:
    try:
        from scripts.local_model import call as _local_call, is_available as _local_check
        _LOCAL_AVAILABLE = True
    except ImportError:
        _LOCAL_AVAILABLE = False
        def _local_call(*a, **kw): raise ImportError("local_model 未安装")
        def _local_check(): return False
...
try:
    from secrets import DEEPSEEK_API_KEY, CATROUTER_API_KEY, DEEPSEEK_BASE_URL, CATROUTER_BASE_URL
except ImportError:
...
_COMPILED_RULES = {
    tier: [re.compile(p) for p in patterns]
...
def route_task(prompt: str, task_type: str = "",
               ...
               allow_local: bool = True) -> ModelTier:
    """
...
def get_model(prompt: str, budget_status: dict = None,
              task_type: str = "", force_tier: ModelTier = None,
              local_available: bool = None) -> dict:
    """
    ...
    if budget_status:
        ...
        if budget_tier == "flash_only":
            ...
            if tier in (ModelTier.PRO, ModelTier.PREMIUM):
                print(f"   ⛔ budget_guard: {tier} 被降级为 Flash（¥{budget_status['spent']:.0f}/¥400已用）")
...
def _parse_success_response(body: dict, model: str, provider: str,
                            cost_per_10k: float, task: str, project: str) -> dict:
    ...
    _try_log_call(model, input_tokens, output_tokens, task, project,
...
def call_llm(prompt: str, model_config: dict,
             system: str = None, temperature: float = 0.3,
             max_tokens: int = 4096,
             task: str = "", project: str = "Claw") -> dict:
    """
    统一的 LLM API 调用函数。
    ...
    if provider == "ollama":
        try:
            ...
            if local_result["success"]:
                _try_log_call(model, local_result.get("prompt_tokens", 0),
            ...
            return _make_error_resp(model, "ollama", elapsed,
                                    local_result.get("error", "Ollama 调用失败"))
        except ImportError:
            return _make_error_resp(model, "ollama",
...
def _try_log_call(model, input_tokens, output_tokens, task, project,
                  hit_tokens=None, miss_tokens=None):
    """安全地记录成本日志（不抛出异常）"""
    try:
        _log_call(model, input_tokens, output_tokens,
...
def call_with_fallback(prompt: str, task_type: str = "",
    ...
    candidates = [primary] + [
        {"model": c["model"], "provider": c["provider"],
         "base_url": CATROUTER_BASE_URL if c["provider"] in ("catrouter", "premium") else DEEPSEEK_BASE_URL,
    ...
    for idx, candidate in enumerate(unique_candidates):
        ...
        result = call_llm(prompt, mc, system=system,
                          temperature=temperature, max_tokens=max_tokens,
                          task=task or task_type, project=project)
        ...
        if result["success"]:
            ...
            sleep_sec = min(8, (2 ** idx) * 0.5 + random.uniform(0, 0.3))
...
if __name__ == "__main__":
    ...
    if cmd == "test":
        ...
        mc = {"model": model, "provider": provider,
        ...
        if result["success"]:
            ...
            if result.get("prompt_cache_hit_tokens"):
                print(f"   📦 Cache: hit={result['prompt_cache_hit_tokens']} miss={result['prompt_cache_miss_tokens']}")
        else:
        ...
        if result["success"]:
            ...
            for i, a in enumerate(result.get("attempts", [])):
                print(f"   #{i+1} {a['model']}: {'✅' if a['success'] else '❌'} {a.get('error', '')[:80]}")
    elif cmd == "budget":
        # 测试预算感知调用
        ...
        print("用法: python router.py [test|route '<prompt>'|call '<prompt>' [task] [provider] [model]|fallback '<prompt>' [task]|budget '<prompt>']")
