...
- [ ] 此类「orchestrator 重启打断 + 夜间整窗丢档」已 ≥2 次同类发生 → ✅已升级(2026-08-16)为🔴铁律「双自动化并发操作同一目标须互斥」(互斥/单一配置源/写前2min护栏)。中枢重跑兜底建议（看门狗超阈值后自动触发中枢重跑 / 本地 unified_ops_center.py --no-push 兜底刷新心跳）仍待落地。
